...
// addr1　素材
  ...
  val nex_addr = (addr_block + 1.U) << blockOffset.U
...
//addr2 素材
  ...
  */
  ...
  // val data_bytes = VecInit(Seq.tabulate(cacheDataBits/8) { i => recv_data2(8 * (i + 1) - 1, 8 * i) })

  when(state === s_process){

  printf(p"s_process\n")

  for(i <- 0 until data_bytes.length - 1){
